...
int TCPServer::WriteBlock(map<string, string> &map,list<string> keylist)
{
	...
	if (to_string(len).length()!=to_string(newlen).length())newlen++;
...
map<string,int> TCPServer::login()
{
	...
	if (block_in["Type"]=="ParameterAuthenticate")
	{
		...
		int ret=UserAuthenticate(auth_str);
		if (ret==-1)//登录失败
		{
...
int TCPServer::UserAuthenticate(string auth_str)
{
	printf("auth_str=%s\n",auth_str.c_str());
	return 0;
}
